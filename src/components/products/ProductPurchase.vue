...
export default {
    ...
    beforeRouteLeave (to, from, next) {
       if ((this.product.title.length > 0 || this.product.count > 0 || this.product.price > 0 || this.product.description.length > 0 ) && !this.saveButonClick) {
           if (confirm("Ürün bilgilerini kaydetmeden çıkmak istediğinize emin misiniz?")) {
               next();
           }
           else {
               next(false);
